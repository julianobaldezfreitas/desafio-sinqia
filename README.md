...
Algumas notas sobre a interpretaçao do desafio.
...
- Na classe **Arquivo.java** os arquivos são aberto para leitura com padrão de ENCODE UTF-8. Nos teste realizados na aplicação não geram nenhum tipo de erro. Ao implementar os teste unitários automatizados foi percebido que com esta condificação os testes estavam levantando a seguinte exception:
...
- Nesta versão os testes unitários **ArquivoTest** e **ProcessoArquivoTest** estão comentadas **@Ignore** até ter o fix do problema. Para rodar estes teste basta retirar a anotação.
...
- evitar percorrer lista em memória muitas vezes;

## Outras notas:
- Cobertura de testes unitários está em +-60% mesmo com dois teste ignorados conforme citado.
